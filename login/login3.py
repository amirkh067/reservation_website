...
def get_users_db():
    if not hasattr(users_connection_local, 'connection'):
        users_connection_local.connection = psycopg2.connect(database='users.db')
...
def search():
    if request.method == 'POST':
        ...
        cursor.execute('''
            SELECT * FROM users
            WHERE LOWER(username) LIKE LOWER(%s) OR LOWER(email) LIKE LOWER(%s) OR
                  LOWER(hali_sayisi) LIKE LOWER(%s) OR LOWER(phone_number) LIKE LOWER(%s)
