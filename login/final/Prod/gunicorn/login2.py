...
def search():
    if request.method == 'POST':
        ...
        if search_date and not search_term:
            ...
            cursor.execute('''
                ...
                WHERE (LOWER(username) LIKE LOWER(?) OR LOWER(address) LIKE LOWER(?) OR
                      LOWER(hali_sayisi) LIKE LOWER(?) OR LOWER(phone_number) LIKE LOWER(?) OR LOWER(fiyat) LIKE LOWER(?))
                      AND date_added = ?
            ''', (f'%{search_term}%', f'%{search_term}%', f'%{search_term}%', f'%{search_term}%', f'%{search_term}%', search_date))
        ...
        sum_hali_sayisi = sum(result[5] for result in results[1:])  # Ignore the 1st row

        # Calculate the sum of the "Fiyat" column
        sum_fiyat = sum(result[8] for result in results[1:])  # Ignore the 1st row
